...
"""
Run script in Powershell: processdutyemail
...
def ask_for_folder():
    ...
    while len(output_folder) == 0:
        output_folder = input(
            'Please complete the final part of the destination \
                 folder \n(e.g."NGS_401 to 500\\NGS484")\nMay need to \
                     place inside double quotes in Powershell:'
        )
        destination_folder = os.path.join(path_worksheets, output_folder)
        print("The destination folder is : {}".format(destination_folder))
        if destination_folder == path_worksheets or not os.path.isdir(
            destination_folder
        ):
...
def get_data(df, path_to_folder, site_filter="none"):
    ...
    From the df file it obtains the download links
    and creates a string argument.
...
def download_data(all_urls):
    ...
    If it is longer than the character length limit thee string is
    split into multiple smaller string arguments.
...
def save_log_file(text, filename):
    ...
    log_path = "{}\\process_logs\\Finished_on_{}_{}.txt".format(
        config.CSVread_folder, cur_time_string, filename
    )
...
def get_files():
    """
    This function gets the paths for csv files download
    by duty bioinformatician to the Duty_Bioinformatics_CSV folder.
    """
    ...
    for filename in files:
        results = re.search(
            "__(\S+)__", filename
        )  # looks for project type (e.g. WES, SNP, TSO500, MokaPie) in the CSV file name
...
class Project:
    ...
    def process_data(self):
        """
        This function executes the processing of a project based on the project
        type e.g. SNP, WES, TSO500 or MokaPipe
        """
        if self.project == "WES":
            self.process_WES()
    ...
    def process_MokaPipe(self):
        ...
        for file_path in self.project_files:
            ...
            if bool(re.search("NGS\w+_NGS", file_path)):
                print(
                    "WARNING! More than one NGS project has been found in the file name.\n \
                ...
                if StGsearch:
                    ...
                    url_StG_RPKM = get_data(
                        df_RPKM,
                        '"{}"'.format(os.path.join(StG_transfer_folder, "RPKM")),
                    )
                    url_StG_coverage = get_data(
                        df_coverage,
                        '"{}"'.format(os.path.join(StG_transfer_folder, "coverage")),
                        "StG",
                    )
                else:
                    url_StG_RPKM = ""
                ...
                url_coverage = get_data(
                    df_coverage, '"{}"'.format(destination_of_Coverage), "GSTT"
                )
                if len(df_FHPRS.index) >= 1:
                    ...
                    if StGsearch:
                        ...
                        url_StG_FHPRS = get_data(
                            df_FHPRS,
                            '"{}"'.format(os.path.join(StG_transfer_folder, "FH_PRS")),
                            "StG",
                        )
                    else:
                        url_StG_FHPRS = ""
                    url_FHPRS = get_data(
                        df_FHPRS, '"{}"'.format(destination_of_FHPRS), "GSTT"
                    )
                else:
                    url_FHPRS = ""
                    url_StG_FHPRS = ""
    ...
    def archive_files(self):
        ...
        message = "Processed_on_{}".format(cur_time_string)
